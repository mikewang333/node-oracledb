...
class SessionAtts {
  ...
  setFrom(params) {
    if (params) {
      ...
      if (typeof params.walletKey === 'string') {
        this.nt.walletKey = params.walletKey;
      }
  ...
  async prepare(protocol) {
    ...
    if (protocol && (protocol.toUpperCase() == "TCPS" && this.nt.walletKey)) {
      this.nt.wallet = Buffer.from(this.nt.walletKey);
    }
