...
class SessionAtts {
  ...
  setFrom(params) {
    if (params) {
      ...
      if (typeof params.walletPEMContent === 'string') {
        this.nt.walletPEMContent = params.walletPEMContent;
      }
  ...
  async prepare(protocol) {
    ...
    if (protocol && (protocol.toUpperCase() == "TCPS" && this.nt.walletPEMContent)) {
      this.nt.wallet = Buffer.from(this.nt.walletPEMContent);
    }
