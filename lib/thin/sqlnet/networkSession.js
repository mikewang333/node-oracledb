...
async function getConnectionInfo(userConfig) {
  const connStr = await resolveConnectStr(userConfig.connectString, userConfig.configDir, userConfig.tnsnamesContent);
...
async function resolveConnectStr(connectString, configDir, tnsnamesContent) {
  ...
  if ((connStr.indexOf(')') == -1) || (connStr.indexOf('(') != 0)) {
    if ((connStr.indexOf(':') != -1) || (connStr.indexOf('/') != -1)) {
      ...
      if (configDir) {
        ...
        p = await nlParamParser.initializeNlpaWithFilePath(namesFilePath);
      } else {
        p = await nlParamParser.initilializeNlpaWithStrContent(tnsnamesContent);
      }
...
async function resolveAddress(connStr, configDir, tnsnamesContent) {
  const connstr = await resolveConnectStr(connStr, configDir, tnsnamesContent);
  ...
  */
  async connect(userConfig) {
    ...
    if (userConfig._connInfo) {
      ...
      addressNode = await resolveAddress(connStr, userConfig.configDir, userConfig.tnsnamesContent);
    }
    let address;
    try {
      address = await this.getAddress(addressNode, userConfig);
