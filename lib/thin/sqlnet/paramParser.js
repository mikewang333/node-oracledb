 ...
 * @returns {Promise}
 */
  async initializeNlpaWithFilePath(file_path) {
    ...
    return await this.initilializeNlpa(fileContent);
  }

  /**
   * Reads the given string content line by line and stores the
   * network service names mapped to connect descriptors in the hashtable.
   * @param {string} str_content
   * @returns {Promise}
   */
  async initilializeNlpaWithStrContent(str_content) {
    const fileContent = readline.createInterface({
      input: Readable.from(str_content),
      output: process.stdout,
      terminal: false
    });
    return await this.initilializeNlpa(fileContent);
  ...
  async initilializeNlpa(fileContent) {
    this.ht = new Map();
    let nvElem = "";
    for await (let line of fileContent) {
